<?php if (comments_open()): ?>
	...
	<?php if (post_password_required()): ?>
		<p class="nopassword"><?php _e('Post ini dilindungi kata sandi. Masukkan kata sandi untuk melihat.', 'mading'); ?></p>
		<?php return; ?>
	<?php endif; ?>
	...
	<div id="comments">
		<?php if (have_comments()): ?>
			<div class="post-meta">
				<h1 class="alwnor">
					<?php printf(_n('1 komentar', '%1$s komentar', get_comments_number()), number_format_i18n(get_comments_number()), ''); ?>
				</h1>
			</div>
		...
		<?php if ('open' == $post->comment_status): ?>
			...
			<?php comment_form(array(
				'must_log_in' => '<p class="comment_message">' . sprintf(
					'Silahkan <a href="%s">masuk</a> untuk bisa menulis komentar.',
					get_option('siteurl') . '/wp-login.php?redirect_to=' . urlencode(get_permalink())
				) . '</p>',
				'logged_in_as' => '<p class="comment_message">' . sprintf(
					'Masuk sebagai <a href="%s">%s</a>. 
				...
				'comment_notes_after' => '',
				'title_reply' => __('<div class="post-meta"><h1 class="alwnor">tinggalkan komentar</h1></div>', 'mading'),
				'label_submit' => __('Kirim Komentar', 'mading'),
